...
```bash
rsync --dry-run -av \
      /proj/{{< var uppmaxproject >}}/webexport/monkeyflower/small/ .
# Remove the dry run option to actually retrieve data. The dot is
# important!
rsync -av /proj/{{< var uppmaxproject >}}/webexport/monkeyflower/small/ .
```
...
Retrieve the data `{{< var webexport.url
>}}/monkeyflower/small.tar.gz` with `wget`^[The password is provided
by the course instructor]:

```bash
wget --user pgip --password PASSWORD {{< var webexport.url >}}/monkeyflower/small.tar.gz
...
uncomment when copying the code. If you have problems running the
command, it may be because an incompatible `java` version is loaded as
a module. To remedy this situation, run

```{bash }
#| label: module-unload-java
#| echo: true
#| eval: false
module unload java
```
