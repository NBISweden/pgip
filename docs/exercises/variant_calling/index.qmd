...
#| echo: false
#| eval: true
fastqc -o . PUN-Y-INJ*.fastq.gz > /dev/null 2>&1
if [ ! -e PUN-Y-INJ_R1_fastqc ]; then
    unzip PUN-Y-INJ_R1_fastqc.zip > /dev/null 2>&1
fi
if [ ! -e PUN-Y-INJ_R2_fastqc ]; then
    unzip PUN-Y-INJ_R2_fastqc.zip > /dev/null 2>&1
fi
```
...
biology as on getting programs to run and interpreting the outputs.
...
In your course working directory, create an exercise directory and
`cd` to it:
...
FIXME: add data download instructions.
...
![](PUN-Y-INJ_R1_fastqc/Images/per_base_quality.png)
![](PUN-Y-INJ_R2_fastqc/Images/per_base_quality.png)
...
which for $P=0.01$ gives $Q=20$. Hence, a base quality score $Q=20$
corresponds to a 1% probability that the call is incorrect, $Q=30$ a
0.1% probability, and so on. The base qualities typically drop towards
...
{{< fa brands linux >}} Use the command `wc` to determine how many sequences are in `PUN-Y-INJ_R1.fastq.gz`.
...
GATK comes with a large set of tools. For a complete list and documentation, see the @ToolDocumentationIndex_2023.
...
#| echo: false
#| eval: true
Sys.setenv(SRR = "SRR9309790", SAMPLE = "PUN-Y-INJ", REF = "M_aurantiacus_v1.fasta")
```
...
`-ERC` option to generate `GVCF` output format, a condensed VCF format
that includes non-variant sites as well as variant sites. The
`--annotation` (`-A`) option adds annotations to the output that are
used in the subsequent filtering step.
...
data <- read.csv(paste0(sample, ".sort.dup.recal.after.csv"))
ggplot(
...
Change the output file label from `raw` to `recal` (i.e.,
`.sort.dup.raw` becomes `.sort.dup.recal`).
...
`CombineGVCFs`, followed by genotyping with `GenotypeGVCFs`.
...
Create a variant call set `allsites.vcf.gz` but for all the red and
yellow samples.
...
#| echo: true
#| eval: true
cat sampleinfo.csv | grep yellow | cut --fields 1 --delimiter ","
```
...
and more. You can print the header with the command `bcftools view
-h`. An example of each is given below:
