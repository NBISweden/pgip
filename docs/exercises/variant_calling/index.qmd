...
GATK comes with a large set of tools. For a complete list and documentation, see the @_ToolDocumentationIndex_2023.
...
#| echo: false
#| eval: true
Sys.setenv(SRR = "SRR9309790", SAMPLE = "INJ", REF = "M_aurantiacus_v1.fasta")
```
...
Run GATK `HaplotypeCaller` on the recalibrated bam file, followed by
`VariantFiltration`. For the latter, modify the filters to
...
If time permits, run the workflow on some more samples (e.g., `ELF`).
...
#| eval: true
#| results: hide
gatk CombineGVCFs -OVI true --output combine.g.vcf.gz --reference ${REF} \
  -V INJ.sort.dup.recal.hc.g.vcf.gz \
  -V ELF.sort.dup.recal.hc.g.vcf.gz
...
Create `allsites.vcf.gz` but for all the red and yellow samples.
