...
function add_sc_sb_title() {
  ...
  if (Reveal.isReady()) {
    if (
      document.querySelector("div.reveal-header img").getAttribute("src")
        .length == 0
    ) {
    ...
    Reveal.on("slidechanged", (event) => {
      ...
      if (sp != null) {
        ...
        var section_text =
          event.currentSlide.getAttribute("data-sc-title") || " ";
        ...
        var sbsection_text =
          event.currentSlide.getAttribute("data-sb-title") || " ";
        if (event.currentSlide.matches(".title-slide.level1")) {
