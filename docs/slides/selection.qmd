...
#| fig-height: 5
#| fig-width: 10
p <- ggplot(subset(data, fn %in% levels(factor(data$fn))[1:50]), aes(windows, TajD, group=fn)) +
    geom_line(color="lightgray", size=0.5) +
    geom_line(data=data_mean, color="red", size=1.5) +
