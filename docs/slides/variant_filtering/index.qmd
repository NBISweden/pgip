...
mappers and variant
callers.](assets/images/Li-2014-F2.png){#fig-Li-2014-2
...
callers](assets/images/Li-2014-F3.png){#fig-Li-2014-3 fig-align=center
width=70%}
...
## Manual filtering sets thresholds on context statistics {.smallr}
...
upper <- median(data$SUM_DEPTH) + 2*sd(data$SUM_DEPTH)
ggplot(x, aes(x=as.numeric(as.character(Var1)), y=Freq)) + geom_line() +
  xlab("Depth") + ylab("bp") + xlim(0, signif(upper, digits=0)) +
