...
wf_model_igraph <- function(popsize = 10, generations = 10,
    ...
    g <- igraph::add_vertices(g, popsize * generations, allele=NA, show.parent=FALSE, reproduce=NA)
    ...
    attr(grobj, "perm") <- unlist(
        lapply(seq(igraph::vcount(g), 1, - popsize),
               function(x) {
            seq(x - popsize + 1, x)
        }))
    ...
    for (i in seq(1, generations)) {
        ...
        if (i >= 2) {
            ...
            igraph::V(g)[n0:n1]$allele <- V(g)[parents]$allele
            grobj[[length(grobj) + 1]] <- g
